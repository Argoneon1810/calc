...
class Node {
    ...
    func traverse(root: Node) -> Int {
        if root.value == "" {           //For Error Handling
            ExpressionValidator.handleError(textToShow: "Inappropriate value was provided")
        }
        ...
        case 0:                         //if the node owns no child (==value node)
            ...
            let number: Int? = Int(root.value)
            if let temp = number {return temp}
            else {ExpressionValidator.handleError(textToShow: "Number was expected, but it could not be found")}
        ...
        default:                        //For Error Handling
            ExpressionValidator.handleError(textToShow: "Internal Error")
        }
        return Int.min
    }
}
